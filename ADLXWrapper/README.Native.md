...
Use the native path when you need direct ADLX vtable access that mirrors the AMD ADLX SDK samples. You manage pointers and interfaces explicitly; `ComPtr<T>` helps with lifetime. All native APIs live in `ADLXWrapper/cs_generated/` and require `unsafe` code.

> **For most use cases, the Facade layer is recommended.** It is pointer-free, requires no `unsafe` code, and returns `*Dto` objects (e.g. `GpuDto`, `GpuMetricsSnapshotDto`) directly. See `ADLXWrapper/README.md`.
...
- Facade comparison: see `ADLXWrapper/README.md` for the pointer-free equivalents using `*Dto` objects.
