...
try
{
    ...
    var current = perf.GetCurrentGpuMetrics(gpu);
    ...
    var snapshots = perf.EnumerateGpuMetricsHistory(gpu, 0, 0).ToList();
    Console.WriteLine($"GPU history count: {snapshots.Count}");

    var sysSnapshots = perf.EnumerateSystemMetricsHistory(0, 0).ToList();
    Console.WriteLine($"System history count: {sysSnapshots.Count}");

    var allSnapshots = perf.EnumerateAllMetricsHistory(0, 0).ToList();
