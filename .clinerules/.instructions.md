...
This project main aim is to provide C# bindings for the AMD ADLX SDK. You can find the AMD ADLX SDK in the ADLX folder, and it contains an SDKDoc folder that contains all the HTML documents describing the SDK functions, files and features. There is also an ADLXHelper object in the ADLX/ADLXHelper folder, and some useful WinAPI files in the ADLX/Platform/Windows folder.

You should also review the C/C++ samples in the ADLX/Samples folder too as they will be useful for you to know.
...
│   ├── ADLXWrapper.vcxproj   # Visual Studio project file to be able to build the DLL and create the binding files from within Visual Studio
...
├── rebuild_adlx.bat            # Main C++ wrapper rebuild script 
├── test_adlx.bat        # C# test build script
└── swigwin/                    # SWIG installation directory (it is downloaded by the rebuild_adlx.bat script)
```
