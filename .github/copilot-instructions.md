...
CRITICAL: Please do not make the changes until I have given you the permission to proceed. Only initially tell me your plan, and wait for my approval. Your plan description needs to be comprehensive and based on facts - do not make things up. Try to divide work into sensible sized stages, and make sure that you document the plan, including the stages, clearly. I want you to record the plan in a file somewhere that you will refer to while you execute the plan, so that you do not get lost and make mistakes. When you have completed a stage, update the plan to move the stage from the planned status to the completed status. Only proceed to the next stage when I have given you the permission to do so.

DO NOT MAKE UP ANY FACTS. If you don't know the name of a function, then go and check the objects that might have the function in there and find the exact name that it has.  Check the API documentation created by DocFx in the ADLXWrapper/APIDocs folder. If you don't know if something is true or not, try finding the relevant documentation or code that confirms it one way or the other. You are allowed to access web searches to find relevant information, but you must not make up any facts.
...
- **Initialization & lifetimes**: Create the API with `using var adlx = ADLXApi.Initialize();` (or `InitializeWithCallerAdl`). Retrieve services via `GetSystemServicesProfile()` for facades, or `GetSystemServices()` for raw handles; wrap raw pointers in `ComPtr<T>` for RAII. `ADLXApi.Dispose` terminates ADLX then unloads the DLL—calls after disposal throw `ObjectDisposedException`.
...
- **Facade-first**: Prefer facade types (`AdlxDisplay`, `AdlxDesktop`, `ADLXSystemServices` in [ADLXWrapper](../ADLXWrapper)) over legacy static helpers. Legacy helpers remain for compatibility; new work should mirror facade style and immutability.
...
- **Capabilities & gating**: Prefer on-demand `QueryInterface` gating at call sites: when a feature needs a newer interface, attempt `QueryInterface` and propagate `ADLX_NOT_SUPPORTED`/`ADLX_BAD_VER` from the SDK rather than hard-disallowing older stacks. For version strings use `ADLXApi.GetVersion()` / `GetFullVersion()`. Display apply paths may skip DS3-only items with callbacks. Always guard hardware access with `ADLXHardwareDetection.HasAMDGPU` and `ADLXApi.IsADLXDllAvailable` for clean skips.
...
- **API docs**: DocFX site under `APIDocs/` (config in `APIDocs/docfx.json`, content in `APIDocs/docs/`, output `_site`). Generate with `docfx docfx.json --serve` from `APIDocs` (DocFX required).
